...
class AuthManagerTest(IntegrationSetup):
    ...
    def test_authenticate_authenticates_user(self):
        ...
        rt_payload = self.refresh_token_manager.verify_payload(validation_token.get_refresh_token().get_token())
    ...
    def test_generate_validation_token_generates_new_tokens(self):
        ...
        validation_token = self.auth_manager.authenticate(user_info["email"], user_info["password"])
        time.sleep(1)
        nv_token = self.auth_manager.generate_validation_token(validation_token.get_refresh_token().get_token())

        with self.assertRaises(RefreshTokenFetchException):
            self.refresh_token_manager.get(validation_token.get_refresh_token().get_token())
            self.fail("Did not fail on fetch for deleting old refresh token on generation")

        self.assertNotEqual(validation_token.get_refresh_token().get_token(), nv_token.get_refresh_token().get_token())
        self.assertNotEqual(validation_token.get_access_token(), nv_token.get_access_token())
        self.assertNotEqual(validation_token.get_refresh_token().get_id(), nv_token.get_refresh_token().get_id())
        self.assertEqual(validation_token.get_refresh_token().get_user_id(), nv_token.get_refresh_token().get_user_id())
    ...
    def test_generate_validation_token_fails_on_non_active_user_status(self):
        ...
        with self.assertRaises(AuthStatusException):
            self.auth_manager.generate_validation_token(validation_token.get_refresh_token().get_token())
